...
def get_spans(df, partition, scale=None):
    ...
    for column in df.columns:
        if column in categorical:
            span = len(df[column][partition].unique())  # 字符串长度（去重）
        else:
            span = df[column][partition].max() - df[column][partition].min()  # 数值范围
...
rects = get_partition_rects(
    df, finished_partitions, column_x, column_y, indexes, offsets=[0.0, 0.0]
)
